...
class TestDropGovernedTaggedColumn(RerunSafeMixin, MaterializationV2Mixin):
    """Prove DROP COLUMNS works when the column carries a *governed* tag.

    Ordinary ``databricks_tags`` do not raise ``CANNOT_DROP_TAGGED_COLUMN``; that
    error is specific to governed tags. Creating a governed tag requires
    account-level ``CREATE``, so environments without that privilege soft-skip
    rather than failing (or worse, passing without exercising the bug).
    """
    ...
    def governed_tag_key(self):
        # Unique account-scoped key so parallel CI jobs do not collide.
        # Intentionally independent of ``project`` to avoid a fixture cycle
        # (models -> tag key -> project -> models).
    ...
    def ensure_governed_tag(self, project, governed_tag_key):
        ...
        try:
            ...
            pass  # best-effort cleanup; leftover policy is account-scoped noise
    ...
    def test_drop_governed_tagged_column(self, project, governed_tag_key):
        ...
        # Without unset-before-drop this fails with CANNOT_DROP_TAGGED_COLUMN.
        util.run_dbt(["run"])
